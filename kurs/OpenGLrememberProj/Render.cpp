...
void initRender(OpenGL *ogl)
{
	...
	*/
	...
	glActiveTexture(GL_TEXTURE0);
	loadModel("models\\plant.obj", &objModel);
	objModelTex.loadTextureFromFile("textures//rTex.bmp");
	objModelTex.bindTexture();

	glActiveTexture(GL_TEXTURE0);
	loadModel("models\\han.obj", &car);
	carTex.loadTextureFromFile("textures//texte.bmp");
	carTex.bindTexture();

	glActiveTexture(GL_TEXTURE0);
	loadModel("models\\plant.obj", &monkey);
	monkeyTex.loadTextureFromFile("textures//sTex.bmp");
...
{   
	...
	*/
	...
	//objModel.DrawObj();
	s[2].UseShader();
	int l = glGetUniformLocationARB(s[2].program, "tex");
	...
	s[2].UseShader();
	int z = glGetUniformLocationARB(s[2].program, "tex");
	glUniform1iARB(z, 0);     //так как когда мы загружали текстуру грузили на GL_TEXTURE0
	glPushMatrix();
	glTranslated(0, 0, Time * 0.1);
	glRotated(-90+Time * 10, 0, 0, 1);
	carTex.bindTexture();
	car.DrawObj();
	glPopMatrix();
	...
	s[2].UseShader();
	int a = glGetUniformLocationARB(s[2].program, "tex");
	glUniform1iARB(a, 0);     //так как когда мы загружали текстуру грузили на GL_TEXTURE0
	glPushMatrix();
